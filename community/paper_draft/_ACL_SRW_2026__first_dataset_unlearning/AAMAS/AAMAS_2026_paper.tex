...
\title[The Silence of the Facts: Popularity as a Barrier to Machine Unlearning]{The Silence of the Facts: Popularity as a Barrier to Machine Unlearning}
...
\begin{abstract}
Machine Unlearning (MU) is an increasingly critical capability for Large Language Model (LLM) \textcolor{red}{agents}, enabling the removal of unsafe, outdated, or private information. However, existing unlearning methods are often evaluated under the assumption that all facts are equally challenging to forget. \textcolor{red}{Since controllable removal of knowledge is essential for reliable, aligned LLM multi-agent systems that operate over long horizons and evolving goals, in} this paper, we \textcolor{red}{the examine evaluation of unlearning LLM agents} from a new angle: \textit{Does the popularity of a fact influence how effectively an LLMs can unlearn it?}

To answer this question, we propose \textbf{UNLamb}, the benchmark designed to systematically investigate this relationship. UNLamb consists of 11.6k question-answer pairs derived from real-world knowledge in Wikidata, explicitly partitioned into rare and popular facts based on entity prominence. Using this benchmark, we perform a comprehensive evaluation of state-of-the-art unlearning algorithms on a set of \textcolor{red}{agentic models} of different sizes. 
We conduct a comprehensive analysis of four unlearning methods: GA, GD, NPO, and RMU, across three validation sets and two LLMs benchmarks. Our results show that larger models struggle more to forget popular facts \textcolor{red}{whose subject entities are highly prominent in Wikidata} and often damage related knowledge when trying to remove them. In contrast, it is much easier to remove rare facts without side effects. Thus, current unlearning methods do not suffice and highlights the need for new strategies that account for the popularity of facts. 
\textcolor{red}{The experiments and detailed analysis presented here provide a foundation for adapting future unlearning algorithms to incorporate fact popularity as an explicit signal, enabling more stable and predictable knowledge removal in agentic LLM systems.}
\end{abstract}
...
\keywords{Machine unlearning, Evaluating LLM-agents, Benchmarking}
...
Large Language Models (LLMs) are central to modern \textcolor{red}{multi-agent systems \cite{li2024survey,lymperopoulos2025tools}}, yet their training on massive web datasets often embeds private, copyrighted, or erroneous information \cite{wei2024evaluating, jang2023knowledge, NEURIPS2024_b1f78dfc}. 
...
The goal is to excise a targeted subset of training data so thoroughly that the model behaves as if those examples were never seen, while preserving its overall competence and avoiding the prohibitive cost of full retraining \cite{geng2025comprehensive, jang2023knowledge, zagardo2024more}. \textcolor{red}{Machine unlearning has a wide range of applications in modern LLM-based multi-agent systems since controllable removal of knowledge is important for safe agents that operate over long horizons and evolving goals~\cite{cai2025aegisllm,sanyal2025agents,sanyal2025alu}.}
...
%% в агентных системах также важно разделять знания во избежании конфликтов. таким образом как и редкие, так и популярные знания важно контролировать и учитывать во всех агентах во избежании утечки и конфликта.
\textcolor{red}{
In multi agent settings, reliable knowledge removal also plays a structural role: agents must isolate outdated or inconsistent information to prevent conflicting beliefs from propagating through shared reasoning or communication. This makes controllable unlearning of both rare and popular knowledge essential for maintaining coherent, conflict free internal states across interacting agents.
}
...
Leveraging these annotations, the novel benchmark \textcolor{red}{for evaluating LLM agents}, \textbf{UNLamb} \textit{-- UNlearning LAnguage Models Benchmark}, is introduced, in which we stratify questions by popularity, forming unlearning-style splits—matched \emph{rare}/\emph{popular} forget sets, and keep the remainder for retain/validation sets. 
...
\begin{enumerate}
    \item Propose knowledge popularity as a critical feature for unlearning algorithms and articulate the mechanisms through which it impacts \textcolor{red}{LLM agent} unlearning efficacy.
    \item Publish \textbf{UNLamb}, a novel, popularity-stratified QA benchmark for a more precise evaluation of unlearning methods \textcolor{red}{for LLM agentic systems}, complemented by a public repository to ensure full reproducibility.
...
\textcolor{red}{
RippleEdits \cite{cohen2024evaluating} is also built on PopQA like our proposed benchmark UNLamb, but targets knowledge editing, not machine unlearning. Editing modifies a fact, whereas MU removes the influence of training data, and applying RippleEdits to MU would require additional preprocessing to construct paired forget–retain splits for objectives such as GD, which UNLamb provides by design (See more in Section \ref{sec:benchmark}). RippleEdits also defines “rarity” differently: its subsets are based on Wikidata update time, triplet counts, and top Wikipedia pages, and do not correspond to low-popularity facts. UNLamb instead stratifies entities by monthly page views and treats popularity as a controlled MU variable. This distinction is important in agentic settings, where safe edits and safe removals correspond to different operational requirements.
}
% RippleEdits \cite{cohen2024evaluating} differs from our setting: it studies local edit propagation and consistency of model updates, whereas UNLamb targets popularity-stratified removal with explicit retain and locality checks. 
% This distinction matters for agentic evaluation, where safe edits and safe removals impose different constraints.
% }
...
%% DONE
\begin{table}[h!t]
  \caption{BERTScore on PopQA for 10\% forget-set. Top: rare unlearning splits, Bottom: popular unlearning splits. L8B, LLaMA-3.1 8B model, \textbf{G7B}, Gemma 7B model.}
...
In this work, we constructed the UNLamb benchmark of 11.6k QA pairs to study how fact popularity affects unlearning in LLM \textolor{red}{agentic systems}. Across four methods, three popular/rare forget-set pairs, and three model sizes, we find popular facts harder to remove and likely to cause catastrophic forgetting, while rare facts are erased more cleanly. These results show that popularity matters, expose algorithmic limitations, and lay the foundations for adaptive schemes such as dynamic learning rate selection and popularity-aware method choice to improve unlearning precision and stability.

\textcolor{red}{
Finally, by linking popularity to Wikipedia page-view statistics and to observed model knowledgeability, we provide a practical signal for agentic evaluation pipelines that need predictable, auditable unlearning behavior.
}
...
\textcolor{red}{
Future work is expanding across model families, sizes, and MoE architectures, and pursuing a corpus-level theoretical analysis of exposure statistics in pretraining to explain why certain facts resist parametric unlearning. This direction aligns with recent NeurIPS-style theory on data distributions and memorization \cite{yu2024generalizablity, sanderrethinking, mireshghallah2025position, fan2025opengu}.
}
...
\input{AAAI/text/10_appendix}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% ОБЯЗАТЕЛЬНЫЕ ДОКИ. ОПИСАНИЕ ИЗМЕНЕНИЙ
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
\section*{Summary of Updates}

\textbf{1. Integrating the LLM-Agent Perspective.} The revised version of the paper integrates the theme of LLM-based agents to demonstrate how the proposed machine unlearning framework extends naturally to agentic settings. Corresponding clarifications and additions have been incorporated into the abstract, the introduction (Section~1), the related work review (Section~2), and the main technical sections, including analysis and experiments (Sections~4--7).

\textbf{2. Expanded Experiments and New Semantic Metrics.} \textit{Addresses to AAAI Reviewers RBo7, rbPZ comments:} Additional computational experiments have been conducted: we introduce and compute a cosine semantic similarity metric for both LLaMA and Gemma models, which strengthens the evaluation of how unlearning affects semantic response quality (Sections~4.4--5). The abstract has also been updated to reflect new scaling experiments that further validate the model-size trends reported in the paper.

\textbf{3. Strengthened Justification for Popularity as a Pretraining Signal.} \textit{Addresses to AAAI Reviewers rbPZ, Giab, RBo comments:} The methodological justification for using Wikidata-derived popularity scores has been strengthened, including an analysis of their connection to pretraining exposure frequencies and, consequently, to the likelihood of parametric memorization by large language models (Section~4).

\textbf{4. Substantial Expansion of Literature Review and Benchmark Context.} \textit{Addresses to AAAI Reviewers rbPZ, Giab, RBo, 2cpk, Meta comments:} The literature review has been substantially expanded:
   \begin{itemize}
       \item the rationale behind the choice of unlearning algorithms has been clarified,
       \item additional relevant benchmarks and studies on machine unlearning and knowledge editing have been incorporated,
       \item recent research on the correlation between fact popularity and parameterized memory in LLMs has been discussed,
       \item several open research questions have been identified to highlight the importance and timeliness of this direction.
   \end{itemize}
   The expanded review incorporates recent results from major conferences, including NeurIPS and related venues (Sections~4--7).

\textbf{5. Reinforced Conclusions and Theoretical Grounding of Contributions.} \textit{Addresses to AAAI Reviewers sbEk, Meta comments:} The conclusion section has been strengthened: the significance of our contribution is now supported by empirical evidence from the extended experimental suite and by theoretical insights from prior work in leading conferences. We emphasize that, although fact popularity may appear intuitive, its impact on unlearning stability exhibits a complex structure, confirmed by the experimental patterns uncovered in our study. Our paper complements existing theoretical work with a practical perspective, offering actionable guidance derived from extensive empirical evaluation (Sections~6--7).

\textbf{6. Expanded Future Work.} \textit{Addresses to AAAI Reviewers Giab, RBo7 comments:}

The future work section has been extended with additional forward-looking research directions. These include planned experiments with Mixture-of-Experts architectures, comparative studies across diverse LLM families, and deeper investigation of how model design impacts unlearning robustness.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% ОБЯЗАТЕЛЬНЫЕ ДОКИ. РЕЛЕВАНТОСТЬ ТОПИКА
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section*{Relevant for the AAMAS research community}
This work is directly relevant to the \textbf{Generative and Agentic AI (GAAI) track} (especially \textit{Evaluation of LLMs/agentic system}s topic) because machine unlearning is a foundational mechanism for maintaining consistency, safety, and controllability in LLM-based agents. Unlearning enables an agent to isolate or remove specific knowledge without destabilizing its remaining capabilities, thereby preventing conflict between outdated, unsafe, or contradictory information that may otherwise propagate through multi-agent communication. Although our experiments focus on a single LLM agent, this setting naturally extends to multi-agent systems, where each agent must adjust or revoke internal knowledge independently to ensure coherent joint behavior. A key contribution of the paper is to show that unlearning robustness is strongly influenced by factual popularity, a parameter that has been overlooked in prior work but critically affects whether an agent can safely excise embedded information. By introducing UNLamb, a popularity-aware benchmark designed to evaluate these failure modes, the paper provides a principled tool for assessing and improving unlearning algorithms in agentic systems, aligning with GAAI themes of agency, safety, and systematic evaluation of generative AI agents.
