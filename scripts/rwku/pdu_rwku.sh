...
echo "[rwku][PDU] Using base checkpoint ${base_model_path}"

experiment="unlearn/rwku/wga_lora.yaml"
trainer="PDU"

output_root="${repo_root}/saves/unlearn/rwku/NEW_BUGFIX_pdu"
mkdir -p "${output_root}"
                ...
                for lora_r in "${lora_rs[@]}"; do
                    for lora_alpha in "${lora_alphas[@]}"; do
                        for lora_dropout in "${lora_dropouts[@]}"; do
                            ...
                            task_name=rwku_${base_model}_${forget_split}_pdu_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_eps${eps_tag}_alpha${alpha_tag}_gamma${gamma_tag}
                            run_dir=${output_root}/${task_name}
                            eval_dir=${run_dir}/evals
                            summary_path=${eval_dir}/DUET_SUMMARY.json

                            if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                echo "[rwku][PDU] Skipping ${task_name}: found existing summary at ${summary_path}"
                                continue
                            fi
