...
#   trainable_params_regex: [".*lora.*"]  -- only LoRA adapter weights are updated
#   retain_loss_type: EMBED_DIFF    -- hidden-state MSE vs reference model
# The reference model is a deepcopy of the PEFT model made at init time.
experiment="unlearn/duet/wga_lora.yaml"
trainer="RMU_lora"

output_root="${repo_root}/saves/unlearn/duet/NEW_BUGFIX_rmu"
mkdir -p "${output_root}"
...
# steering_coeff: magnitude of the random control vector (RMU paper default: 20 for full model,
# lower values like 2 work better with LoRA since only adapters are updated)
raw_steerings="${STEERING_COEFFS:-2}"
raw_steerings="${raw_steerings//,/ }"; raw_steerings="${raw_steerings//\"/}"; raw_steerings="${raw_steerings//\'/}"
read -r -a steerings <<< "${raw_steerings}"
...
for split in "${forget_retain_splits[@]}"; do
    ...
    for lr in "${lrs[@]}"; do
        for steering in "${steerings[@]}"; do
            steering_tag=${steering//./p}
                    ...
                    for lora_r in "${lora_rs[@]}"; do
                        for lora_alpha in "${lora_alphas[@]}"; do
                            for lora_dropout in "${lora_dropouts[@]}"; do
                                ...
                                task_name=duet_${base_model}_${forget_label}_rmu_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_sc${steering_tag}_alpha${alpha_tag}_gamma${gamma_tag}
