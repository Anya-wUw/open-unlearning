...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[duet][PDU] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    echo "[duet][PDU] Using Hugging Face base checkpoint ${base_model_path}"
fi

# PDU: Primal-Dual Unlearning.
# retain_loss_eps is the feasibility threshold on the retain loss.
# The dual variable (retain preference weight) increases whenever retain_loss > eps,
# effectively enforcing a soft constraint on retain quality.
# Typical starting value: 0.3 (paper default for TOFU).
# The dual controller starts updating after dual_warmup_epochs.
experiment="unlearn/duet/wga_lora.yaml"
trainer="PDU"

output_root="${repo_root}/saves/unlearn/duet/NEW_BUGFIX_pdu"
mkdir -p "${output_root}"
...
# retain_loss_eps: the target retain-loss ceiling.
# Dual weight increases when retain_loss > eps; try 0.1–0.5 depending on model.
raw_epss="${RETAIN_LOSS_EPS:-0.3}"
raw_epss="${raw_epss//,/ }"; raw_epss="${raw_epss//\"/}"; raw_epss="${raw_epss//\'/}"
read -r -a epss <<< "${raw_epss}"
...
dual_step_size=${DUAL_STEP_SIZE:-1.0}
dual_warmup_epochs=${DUAL_WARMUP_EPOCHS:-3}
                    ...
                    for lora_r in "${lora_rs[@]}"; do
                        for lora_alpha in "${lora_alphas[@]}"; do
                            for lora_dropout in "${lora_dropouts[@]}"; do
                                ...
                                task_name=duet_${base_model}_${forget_label}_pdu_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_eps${eps_tag}_alpha${alpha_tag}_gamma${gamma_tag}
                                run_dir=${output_root}/${task_name}
                                eval_dir=${run_dir}/evals
                                summary_path=${eval_dir}/DUET_SUMMARY.json

                                if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                    echo "[duet][PDU] Skipping ${task_name}: found existing summary at ${summary_path}"
                                    continue
                                fi

                                echo "${task_name}: PDU LoRA unlearning ${base_model_path} on ${forget_split}"

                                adapter_path=${run_dir}/adapter_model.safetensors
                                log_file=${run_dir}/pdu.log
