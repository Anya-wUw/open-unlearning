...
# UNDIAL uses LoRA's disable_adapter() as its reference model,
# so no separate ref_model copy is needed (memory-efficient).
experiment="unlearn/duet/wga_lora.yaml"
trainer="UNDIAL"

output_root="${repo_root}/saves/unlearn/duet/NEW_BUGFIX_undial"
mkdir -p "${output_root}"
...
# beta: penalty strength on memorised tokens (recommended range 3–30)
raw_betas="${BETAS:-3}"
raw_betas="${raw_betas//,/ }"; raw_betas="${raw_betas//\"/}"; raw_betas="${raw_betas//\'/}"
read -r -a betas <<< "${raw_betas}"

# alpha: weight on the retain NLL loss (0 = forget-only, 0.1 = light regularisation)
